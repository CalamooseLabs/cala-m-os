...
in {
  ...
  config = lib.mkIf (cfg.seedSource != null) {
    # Seed-if-absent. `cp -n` skips files the machine already owns;
    # `--no-preserve=mode` drops the 0444 store bit so OBS can rewrite the copy.
    ...
    home.packages =
      ...
      ++ lib.optional (cfg.repoPath != null) (
        pkgs.writeShellApplication {
